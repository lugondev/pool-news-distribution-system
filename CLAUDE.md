...
4. **Scheduled webhook job** (default every 1 min): checks SQLite for due webhook schedules (cron-based) → fetches articles by filter → dispatches to configured endpoints. See `docs/WEBHOOK_SCHEDULING.md` for details.
...
- **Webhook scheduling**: Cron-based scheduled webhook triggers managed via UI. SQLite stores schedules, APScheduler executes every minute. See `docs/WEBHOOK_SCHEDULING.md` for details.
